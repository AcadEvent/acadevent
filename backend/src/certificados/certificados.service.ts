...
export class CertificadosService {
  ...
  async gerarPdfCertificado(codigoAutenticidade: string): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const nomeAtividade = cert.nome_atividade || 'Atividade Academica';
      const nomeEvento = cert.edicao.titulo_oficial;
      const cargaHoraria = cert.carga_horaria_impressa || '4';
      ...
      doc
        ...
        .fillColor('#777777')
        .text(
          `Valide este certificado em: http://localhost:3001/validar/${cert.codigo_autenticidade}`,
          { align: 'center' },
        );
